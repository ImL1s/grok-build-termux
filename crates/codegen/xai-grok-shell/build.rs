...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Skip auto-bundling on Windows and Android: ripgrep ships .zip on Windows,
    // and Android/Termux resolves native packages from $PATH / $PREFIX/bin.
    // Returning here BEFORE emitting `cargo:rustc-cfg=bundle_rg` keeps include_bytes!
    // macros gated on cfg(bundle_rg) compiled-out, so the runtime falls back to
    // `rg` on PATH. An explicit GROK_SHELL_BUNDLE_RG_PATH still bundles regardless of target.
